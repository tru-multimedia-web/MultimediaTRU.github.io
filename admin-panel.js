...
function setupEventListeners() {
    ...
    document.getElementById('videoUrl').addEventListener('blur', validateYouTubeURL);
}
...
function loadVideos() {
    ...
    if (stored) {
        ...
        saveVideos();
        console.log('📚 Loaded initial demo videos');
    }
}
...
function validateYouTubeURL() {
    ...
    if (url && !getYouTubeVideoId(url)) {
        showAlert('กรุณาใส่ YouTube URL ที่ถูกต้อง', 'error');
        input.focus();
        return false;
    }
    return true;
...
function handleAddVideo(event) {
    ...
    if (!videoId) {
        showAlert('YouTube URL ไม่ถูกต้อง กรุณาตรวจสอบอีกครั้ง', 'error');
        return;
    }
            ...
            <div class="video-item">
                <div class="video-item-header">
                    <div class="youtube-preview">
                        <img src="${getYouTubeThumbnail(video.videoId)}" alt="Video thumbnail">
                    </div>
    ...
    const videoItems = videos.map(video => `
                <div class="video-item" data-category="${video.category}">
                    <div class="video-container">
                        <iframe 
                            src="https://www.youtube.com/embed/${video.videoId}?rel=0&modestbranding=1&controls=1&fs=1&cc_load_policy=0&iv_load_policy=3" 
                            frameborder="0" 
                    ...
                    <div class="video-info">
                        ...
                        <span class="student-name">นักศึกษา: ${video.studentName}</span>
                    </div>
                </div>`).join('\n');
